...
int EView::vGrep(ExState &State) { // lechee
    ...
    // Use "--color never" and "--heading" to group matches by file for better UX.
    snprintf(DoCmd, sizeof(DoCmd),
             "sh -c 'R=$(git -C \"$1\" rev-parse --show-toplevel 2>/dev/null); if [ -z \"$R\" ]; then R=\"$1\"; fi; rg -n --heading --color never \"$2\" \"$R\" > /tmp/fte.grp' -- \"%s\" \"%s\"",
             Dir, safeCmd);
